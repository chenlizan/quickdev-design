...
const Catcher: React.FC<CatcherProps> = ({ dom = document.createElement('div'), domRect, type }) => {
  ...
  if (type === 'select') {
    return (
        <div className={classNames('catcher', 'select')} style={styles}
             onMouseEnter={() => setDrag(true)} onMouseOut={() => setDrag(false)}>
    ...
    return (
        <div className={classNames('catcher', 'hover')} style={styles}/>
    );
  }
};
